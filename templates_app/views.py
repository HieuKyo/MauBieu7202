...
@login_required
def boundary_lookup_view(request):
    """
    Trang tra cứu địa bàn hành chính sau sáp nhập
    """
    from .boundary_service import boundary_service

    # Lấy thông tin tóm tắt
    summary = boundary_service.get_summary()
    provinces = boundary_service.get_all_provinces()
    data_sources = boundary_service.get_data_sources()
    ...
    return render(request, 'templates_app/boundary_lookup.html', context)


@login_required
def boundary_search_api(request):
    """
    API tìm kiếm địa bàn thay đổi
    GET params:
        - q: từ khóa tìm kiếm
        - type: loại tìm kiếm (province, change, old_to_new)
    """
    from .boundary_service import boundary_service

    query = request.GET.get('q', '').strip()
    search_type = request.GET.get('type', 'change')

    try:
        if search_type == 'province':
            # Tìm kiếm theo tỉnh
            results = boundary_service.search_provinces(query)
            return JsonResponse({
                'success': True,
                'type': 'province',
                'count': len(results),
        ...
        elif search_type == 'old_to_new':
            # Tra cứu địa bàn cũ -> mới
            result = boundary_service.get_old_to_new_mapping(query)
            return JsonResponse({
                'success': True,
                'type': 'old_to_new',
                'result': result
            ...
            # Tìm kiếm thay đổi (mặc định)
            results = boundary_service.search_changes(query)
            return JsonResponse({
                'success': True,
                'type': 'change',
                'count': len(results),
...
@login_required
def boundary_provinces_api(request):
    """
    API lấy danh sách tỉnh/thành có thay đổi
    GET params:
        - code: mã tỉnh (optional)
    """
    from .boundary_service import boundary_service

    province_code = request.GET.get('code', '').strip()

    try:
        if province_code:
            # Lấy tỉnh cụ thể
            province = boundary_service.get_province_by_code(province_code)
            if province:
                # Lấy cả các thay đổi của tỉnh
                changes = boundary_service.get_changes_by_province(province.get('name', ''))
                return JsonResponse({
                    'success': True,
                    'province': province,
                    'changes': changes
                ...
                return JsonResponse({
                    'success': False,
                    'error': f'Không tìm thấy tỉnh với mã {province_code}'
                }, status=404)
        else:
            # Lấy tất cả tỉnh
            provinces = boundary_service.get_all_provinces()
