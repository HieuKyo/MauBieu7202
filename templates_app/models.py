...
class DocumentHistory(models.Model):
    """
    Lịch sử tạo mẫu biểu - Audit log cho việc tạo tài liệu
    """
    ...
    template_name = models.CharField(max_length=200, verbose_name="Tên mẫu biểu")  # Backup nếu template bị xóa
    ...
    customer_name = models.CharField(max_length=200, blank=True, verbose_name="Tên khách hàng")  # Backup
    customer_cmnd = models.CharField(max_length=20, blank=True, verbose_name="CMND khách hàng")  # Backup
    ...
    created_by_username = models.CharField(max_length=150, verbose_name="Username người tạo")  # Backup
    ...
    # Data snapshot (để xem lại dữ liệu đã điền)
    ...
    def __str__(self):
        return f"{self.template_name} - {self.customer_name or 'N/A'} ({self.created_at.strftime('%d/%m/%Y %H:%M')})"

    @classmethod
    def log_document_creation(cls, template, customer, created_by, file_name, file_size, data_snapshot):
        """
        Tạo log cho việc tạo tài liệu

        Args:
            template: Template object
            customer: Customer object (có thể None)
            created_by: User object
            file_name: Tên file đã tạo
            file_size: Kích thước file (bytes)
            data_snapshot: Dictionary chứa dữ liệu đã điền
        ...
        return cls.objects.create(
            ...
            customer=customer,
            customer_name=customer.ho_ten if customer else '',
            customer_cmnd=customer.so_cmnd if customer else '',
